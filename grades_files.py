...
#   The data of each student is appended to a list with six data fields:
#   Position 0 is last name, 1 is first name, 2 is Homework, 3 is Midterm, 4 is Final exam
#   The group of student lists in the larger "outer" list
#   The resulting report sent to an output file.
...
def calculate_grades():
    ...
    for i in range(len(stu)):
        ...
        midex = int(stu[i][3]) # Midterm exam is pos. 3
        stu[i][4] = stu[i][4][:-1] #Remove the \n from the final exam string (end of the input line)
        finex = int(stu[i][4]) #Final exam is pos. 4
...
def create_report_file():
    ...
    for i in range(len(stu)):
        ...
        repfile.write(sname + "\t" + hw + "\t" + midex + "\t" + finex + "\t" + fingrade + "\t" + stu[i][6] + "\n")
    repfile.write(line)
    repfile.close()
