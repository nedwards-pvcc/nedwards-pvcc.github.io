...
def main():
    ...
    dogs.remove('AnnaBelle')  # fix: should be "dogs" instead of "dog"
    print("\nRemove a dog by name rather than position in the list:")
    print(dogs)

    dogs2 = dogs.copy()  # fix: should use copy() instead of assigning to another variable
    print("\nA list can be copied onto another list by using the copy() method:")
    print(dogs)
    print(dogs2)

    print("\nUse a For loop to give each dog in the list the same last name:")
    for i in range(len(dogs)):
        dogs[i] = dogs[i] + " Edwards"
    print(dogs)


main()  # fix: should be called outside of the function definition
